...
export class MenuItemInner extends React.PureComponent<MenuItemInner.Props> {
  ...
  private triggerHandler(e?: MouseEvent) {
    if (!this.props.disabled && !this.props.hidden) {
      if (this.props.name) {
        console.log('有name调用Menu的上下文中的onClick函数')
        this.props.context.onClick(this.props.name, e)
      }

      if (this.props.onClick) {
        console.log('手动调用MenuItem实例.props.onClick')
        this.props.onClick()
      }
    }
  }
  ...
  render() {
    // const ps = MenuItemInner.getProps(this.props)
    // console.log(ps)
    // const content = MenuItemInner.getContent(this.props, this.onClick)
    // console.log(content)
    return (
